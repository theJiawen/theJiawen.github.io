...
表单几乎是前端工程师写的最多的东西，登录注册、修改用户信息都离不开表单。在提交一个表单给后台之前，客户端一般要对一些数据进行校验，比如Email、电话是否符合格式，某些必填字段是否为空等等。
...
可以看到，在`onsubmit`函数中，我们对`registerForm`的每个字段逐一验证，这是十分符合逻辑的写法。但是略有经验的工程师会知道，将来我们可能要新增一些字段，比如邮箱；还有可能要对某些字段新增校验规则，比如要求密码必须同时包含大小写字符和数字。这样一来，我们就要经常对`onsubmit`函数进行修改。另外我们也可以注意到，这里也是有很多个`if`哦，应该是可以优化的。
...
```
那么**环境类**呢？其实在 JavaScript 随便一个函数就可以了。

简而言之，我靠..写到最后发现 JavaScript 中根本没有策略模式。基本上只要你熟悉了通过一个对象的方式来避免某些情况下的多个`if`，你就算是掌握了策略模式了。
